        ...
        <div class="row">
            <div class="col-lg-3 m-auto">
                <div class="about-inner-header heading9 text-center">
                    <h2>My Portfolio</h2>
                    <a href="{{ route('home') }}">Home <i class="fa-solid fa-angle-right"></i>
            ...
            @foreach($categories as $index => $category)
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ $index === 0 ? 'active' : '' }}" id="tab-{{ $category->id }}"
                        ...
                        aria-controls="category-{{ $category->id }}" aria-selected="{{ $index === 0 ? 'true' : 'false' }}">
                    ...
                    <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="category-{{ $category->id }}"
                        ...
                        <div class="row">
                            @foreach($category->works as $work)
                                            ...
                                            @php
                                                $isPerformanceMarketing = $category->category_name === 'Performance Marketing';
                                            @endphp
                                            <div
                                                class="{{ $isPerformanceMarketing ? 'col-lg-6 col-md-6 col-sm-12' : 'col-lg-3 col-md-4 col-sm-6' }} mb-4">
                                                <div class="portfolio_card shadow position-relative">
                                                    <div class="portfolio-item">
                                                        <img src="{{ asset($work->image) }}" alt="{{ $work->title }}" class="img-fluid"
                                                            style="{{ $isPerformanceMarketing ? 'width: 100%;' : 'height: 300px;' }}">
